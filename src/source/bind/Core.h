...
void render(Document &doc, Type<PairMatrix<T>> t) {
    ...
    doc.method(t, "permute", [](PairMatrix<T> const &m, Col<std::uint32_t> const &x) { // x is a map of old to new
        auto u = m;
        izip(m.diagonal, [&](auto i, auto d) {u.diagonal[x[i]] = d;});
        auto entries = sorted(vmap(indices(m.values), [&](auto i){return std::make_tuple(x[m.rows[i]], x[m.cols[i]], m.values[i]);}));
        zip(entries, u.values, u.rows, u.cols, [](auto const &t, auto &v, auto &r, auto &c) {std::tie(v, r, c) = t;});
        return u;
    });
    render_public(doc, t);
    render_json(doc, t);
