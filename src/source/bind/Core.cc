    ...
    doc.method(t, "new", [](std::string_view s, std::optional<std::string_view> mat) -> Alphabet {
        if (mat && !mat->empty()) {
            ...
            if (!r && !d){
                if (t && !u) return DNA;
                if (u && !t) return RNA;
                // if (u && t) return RNADNA; This is prevented now, allow when mixed materials are actually supported
                return RNA;
            } else{
                if (r && !d) return RNA;
                if (d && !r) return DNA;
...
void render_compare_key(Document &doc, Type<T> t) {
    doc.method(t, "<",  [](T const &a, T const &b) {auto const A = F::get(a), B = F::get(b); if (A == B) F::check_equal(a, b); return A <  B;});
    doc.method(t, ">",  [](T const &a, T const &b) {auto const A = F::get(a), B = F::get(b); if (A == B) F::check_equal(a, b); return A >  B;});
    doc.method(t, "<=", [](T const &a, T const &b) {auto const A = F::get(a), B = F::get(b); if (A == B) F::check_equal(a, b); return A <= B;});
    doc.method(t, ">=", [](T const &a, T const &b) {auto const A = F::get(a), B = F::get(b); if (A == B) F::check_equal(a, b); return A >= B;});
    doc.method(t, "==", [](T const &a, T const &b) {auto const A = F::get(a), B = F::get(b); if (A == B) F::check_equal(a, b); return A == B;});
...
std::atomic<int> ComplexWarning{0};

template <class T>
struct ComplexCompare {
    static auto get(T const &x) {return lowest_rotation(x.strands);}
    static auto check_equal(T const &x, T const &y) {
        if (!x.name.empty() && !y.name.empty() && x.name[0] != '(' && y.name[0] != '(' && x.name != y.name)
            if (ComplexWarning++ == 0)
                print(fmt::format("Warning (NUPACK): complexes {} and {} appear identical but have different user-chosen names. Future warnings of this type will be suppressed.", x.name, y.name));
    }
};
...
void render(Document &doc, Type<NamedComplex> t) {
    ...
    doc.method(t, "nicks", [](NamedComplex const &c, bool include_zero) {return complex_nicks(c.strands, include_zero);});
    doc.method(t, "symmetry", [](NamedComplex const &c) {return rotational_symmetry(c.strands);});
    render_public(doc, t);
    render_json(doc, t);
    render_compare_key<ComplexCompare<NamedComplex>>(doc, t);
}
...
void render(Document &doc, Type<TargetComplex> t) {
    ...
    render_public(doc, t);
    render_json(doc, t);
    render_compare_key<ComplexCompare<TargetComplex>>(doc, t);
}
...
void render(Document &doc, Type<Structure> t) {
    ...
    doc.method(t, "permute", [](Structure const &s, Col<std::uint32_t> const &old_to_new) {
        NUPACK_REQUIRE(len(old_to_new), ==, len(s));
        NUPACK_REQUIRE(old_to_new.max()+1, ==, len(s));
        PairList p(len(s));
        s.for_each_pair([&](auto i, auto j) {p.toggle_pair(old_to_new[i], old_to_new[j]);});
        // go through old nicks, that gives first bases, then map old to new, that gives new first base locations, then sort
        auto nicks = sorted(vmap<Nicks>(s.nicks, [&](auto n) {return old_to_new[n % len(s)];}));
        return Structure(std::move(p), std::move(nicks));
    });
