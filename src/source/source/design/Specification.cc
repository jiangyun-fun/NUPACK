...
Designer Specification::create_designer() const {
    ...
    for (auto const &[k, v] : complexes) {
        auto const &complex = v.first;
        if (!complex.structure.empty()) {
            DomainList domains;
            for (auto const &s : complex.strands) for (auto const &d : s.domains) domains.emplace_back(d);
            auto const s = join_sequences<Domain>(domains);
            NUPACK_REQUIRE(len(s), ==, len(complex.structure));
            complex.structure.for_each_pair([&](auto i, auto j) {
                bool ok = any_of(s[i].indices(), [&, v=s[j].indices()](auto b) {return any_of(v, [&, b](auto c) {
                    return model.pairing()(Base::from_index(b), Base::from_index(c));
                });});
                if (!ok) {
                    char base_i = model.alphabet()(s[i]);
                    char base_j = model.alphabet()(s[j]);
                    NUPACK_ERROR("An on-target complex structure includes an impossible base pair", i, j, base_i, base_j, complex);
                }
            });
        }
