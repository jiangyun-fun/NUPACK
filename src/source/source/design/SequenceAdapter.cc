...
void DesignSequence::make_sequence(vec<NamedDomain> const &domain_specs, vec<TargetStrand> const &strand_specs) {
    NUPACK_ASSERT(alphabet.data, "empty alphabet in design");
    domains.clear(); strands.clear();
    // constraints = ConstraintHandler();

    vec<Wildcard> total_domain;
    for (auto const &d : domain_specs) {
        NUPACK_ASSERT(d.alphabet.data, "empty domain alphabet in design", d);
        if (d.alphabet == alphabet) {
            extend(total_domain, d);
        } else {
            for (auto b : d) total_domain.push_back(standardize_base(b, d.alphabet, alphabet));
        }
    }
    ...
    constraints = Constraints(alphabet, total_domain);
