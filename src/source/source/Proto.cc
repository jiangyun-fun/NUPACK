...
ParameterFile from_spec(spec::Parameters p, string const &filename) {
    ...
    switch (p) {
        ...
        case spec::Parameters::DNA: {return ParameterFile("dna04.2");}
        ...
        case spec::Parameters::DNA04_1: {return ParameterFile("dna04.1");}
    }
...
std::pair<string, spec::Parameters> get_parameter_name(string_view n) {
    ...
    if (s == "DNA04_1")             return {{}, spec::Parameters::DNA04_1};
...
Model<> from_spec(spec::Model const &m) {
    ModelConditions const c{.temperature=m.kelvin, .na_molarity=m.sodium, .mg_molarity=m.magnesium, .immutable=true};
    ParameterFile const p = from_spec(m.parameters, m.filename);
    Ensemble const e = from_spec(m.ensemble);

    if (!UseModelCache || !c.immutable) return Model(e, p, c);
    static std::list<Model<>> cache;
    static std::mutex mut;
    {
        ParameterInfo info{.file=p, .kind="dG", .na_molarity=c.na_molarity, .mg_molarity=c.mg_molarity, .temperature=c.temperature, .immutable=c.immutable};
        std::lock_guard<std::mutex> lock(mut);
        auto it = std::find_if(cache.begin(), cache.end(), [&](auto const &m) {
            return m.ensemble == e && m.parameters.info == info;
        });
        if (it == cache.end()) {
...
spec::Model to_spec(Model<> const &m) {
    ...
    return {
        .kelvin=m.parameters.info.temperature,
        .sodium=m.parameters.info.na_molarity,
        .magnesium=m.parameters.info.mg_molarity,
        .parameters=p,
