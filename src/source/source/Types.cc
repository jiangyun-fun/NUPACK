...
void to_pairs(iseq *v, iseq const n, char const *dp) {
    ...
    if (s != n - 1) NUPACK_ERROR("unmatched ( parenthesis", dp);
    if (i != n) NUPACK_ERROR("dot-parens-plus parsing failed", dp);
}
...
void disable_noncomplement_closing(BasePairing &p, Alphabet const &a) {
    ...
    izip(p.possible_closing_pairs, [&](auto i, auto &v) {
        v = {a.complement(Base::from_index(i))};
        v.shrink_to_fit();
    });
}
...
BasePairing load_pairing(Alphabet const &a, json const &j) {
    ...
    bp.closing = bp.pairing;
    bp.possible_closing_pairs = bp.possible_pairs;
    return bp;
}
...
json Alphabet::save_repr() const {
    if (*this == DNA) return "DNA";
    if (*this == RNA) return "RNA";
    if (*this == RNADNA) return "RNADNA";
    if (*this == RNA2OMeRNA) return "RNA2OMeRNA";
...
void Alphabet::load_repr(json const &j) {
    ...
    if (j.is_string()) {
        ...
        if (name == "DNA") *this = DNA;
        else if (name == "RNA") *this = RNA;
        else if (name == "RNADNA") *this = RNADNA;
        else if (name == "RNA2OMeRNA") *this = RNA2OMeRNA;
        else NUPACK_ERROR("Unknown alphabet name", name);
    } else {
...
void TargetTube::check() const {
    std::unordered_set<TargetStrand> strand_check;
    for (auto const &x : on_targets()) for (auto const &s : x.strands) strand_check.emplace(s);
    for (auto const &complex : off_targets()) for (auto const &strand : complex.strands)
        NUPACK_ASSERT(strand_check.count(strand), "TargetTube contains an off-target with a strand not contained in any on-target", strand, complex, *this);
...
bool PairList::is_pseudoknotted() const {
    vec<uint> stack;
    for (auto i : indices(values)) {
        if (i < values[i]) stack.emplace_back(i); // left base of last open pair
        else if (i > values[i]) {
            if (values[i] != stack.back()) return true;
            stack.pop_back();
        }
    }
    return false;
...
json PairList::save_repr() const {
    if (is_pseudoknotted()) return values;
    else return dp();
}

void PairList::load_repr(json const &j) {
    if (j.is_string()) *this = PairList(j.get<string>());
    else j.get_to(values);
}

json Structure::save_repr() const {
    if (is_pseudoknotted()) return {{"values", values}, {"nicks", nicks}};
    else return dp();
}

void Structure::load_repr(json const &j) {
    if (j.is_string()) {
        *this = Structure(j.get<string>());
    } else {
        j.at("values").get_to(values);
        j.at("nicks").get_to(nicks);
    }
}
