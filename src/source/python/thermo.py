...
class SampleJob:
    ...
    Note that `seeds` are the real seeds used for the random number generation.
    That is, seeds=[0] has no special meaning. Pass None for a single randomly generated seed
    '''
    number: int
    seeds: List[int]

    def __init__(self, number=1, seeds=None, _fun_=None):
        if seeds is None:
            seeds = [numpy.random.randint(0, 1 << 63, dtype=numpy.int64)]
        assert min(seeds) >= 0, 'Random number generation seed should be non-negative'
        _fun_(self, number, seeds)
...
@forward
def submit(model, jobs, options, _fun_=None):
    ...
    return _fun_(model, jobs, options, gil=False)
...
class StructureEnergy(typing.NamedTuple):
    ...
    def permute(self, order):
        return StructureEnergy(self.structure.permute(order), self.energy, self.stack_energy)
