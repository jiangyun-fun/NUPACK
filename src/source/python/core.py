...
from .rebind import forward, Callable, ConversionError
...
class JSON:
    '''C++ JSON class'''
    def __init__(self, data=None, _fun_=None):
        '''Initialize JSON class from JSON or str'''
        if data is None:
            _fun_(self)
        else:
            try:
                ...
                raise TypeError('Expected nupack.JSON or str but got object of type %s. Suggestion: serialize your argument using json.dumps()' % type(data).__name__) from e
...
class SequenceListBase(Pickleable):
    ...
    def nicks(self, include_zero=False) -> List[int]:
...
class PairMatrix(Pickleable):
    ...
    def permute(self, order, _fun_=None):
        '''Reorder based on specified permutation'''
        return _fun_(self, numpy.asarray(order, dtype=numpy.uint32))
