...
template <class ...Ts>
void print(Ts const &...ts) {((std::cout << ts << " "), ...); std::cout << std::endl;}
...
void symmetric_mdm(M1 &H, M2 const &A, V const &x) {
...
T abstract_bisect(T xlo, T ylo, T xhi, T yhi, T tolerance, F const &f) {
    // accept an improvement that doesn't achieve the tolerance 0.01 seems about right from my timings
    tolerance = std::max(std::abs(ylo) * 0.01, tolerance);
    while (true) {
        T const x = (xlo + xhi) / 2;
        if (x == xlo || x == xhi) return xlo;
        T const y = f(x);
        if (std::abs(y) < tolerance) return x;
        ((std::signbit(y) == std::signbit(ylo) && std::isfinite(y)) ? std::tie(xlo, ylo) : std::tie(xhi, yhi)) = std::tie(x, y);
    }
}

template <class T, class F>
T optimize_scale(T const ylo, T xhi, T const tolerance, F const &f) {
    T yhi = f(xhi);
    if (std::abs(ylo) < tolerance || !std::isfinite(ylo)) return 0;
    if (std::abs(yhi) < tolerance) return xhi;
    while (std::signbit(yhi) == std::signbit(ylo) && std::isfinite(yhi)) {
        xhi *= 2;
        yhi = f(xhi);
    }
    if (!std::isfinite(xhi)) return 0;//throw std::runtime_error("invalid bisection encountered in equilibration");
    T out = abstract_bisect<T>(0, ylo, xhi, yhi, tolerance, f);
...
template <class T, class M>
Output<T> equilibrate_cd(M const &A, la::Col<T> const &logq, Options ops) {
    if (!A.is_finite()) throw std::runtime_error("non-finite equilibration coefficients");
    Output<T> out;
    auto &y = out.dual_solution = std::move(ops.initial_dual_guess);
    la::Col<T> logx = A * y + logq, abs_grad, eigs, y1, x1, logx1, V1, c;
    auto &x = out.solution = la::exp(logx);
    la::Mat<T> H, V, AV(A.n_rows, A.n_cols, la::fill::none), AV2;
    la::uvec order;
    for (out.iters = 0; out.iters != ops.max_iters; ++out.iters) {
        if (!y.is_finite()) throw std::runtime_error("non-finite potential encountered in equilibration");
        if (!x.is_finite()) {
            std::cout << y.t() << std::endl;
            std::cout << x.t() << std::endl;
            throw std::runtime_error("non-finite concentration encountered in equilibration");
        }
        // It was experimented whether the Hessian formulation and diagonalization was better not to do every iteration
        // It seems better to do it every iteration, and it's simpler too
        symmetric_mdm(H, A, x); // O(m n^2)
        if (!H.is_finite()) throw std::runtime_error("non-finite Hessian in equilibrium concentration solver");

        if (true) {
            if (!la::eig_sym(eigs, V, H)) // O(n^3)
                throw std::runtime_error("eigendecomposition failed in equilibrium concentration solver");
        } else {
            // Previous condition: if (la::abs(V.t() * V - la::eye(V.n_cols, V.n_cols)).max() > 1e-10) {
            la::Mat<std::complex<real>> V0;
            la::Col<std::complex<real>> e0;
            if (!la::eig_gen(e0, V0, H, "balance"))
                throw std::runtime_error("general eigendecomposition failed in equilibrium concentration solver");
            eigs = la::real(e0);
            V = la::real(V0);
        }
        V.clean(1e-8); // remove entries close to 0, helps when strands are almost independent
        for (la::uword i = 0; i != V.n_cols; ++i) AV.col(i) = A * V.col(i);
        AV2 = AV % AV;
        V1 = la::sum(V, 0).t();

        // Optimize each direction in order of decreasing gradient
        abs_grad = la::square(AV.t() * x - V1) / eigs; // O(m n)
        for (auto &x : abs_grad) if (!std::isfinite(x)) x = 0;
        order = la::sort_index(abs_grad, "descend");
        out.error = 0;
        for (la::uword i : order) { // O(m n^2) overall
            // Gradient in this DOF
            T const grad = la::dot(AV.col(i), x) - V1(i); // O(m)
            out.error = std::max(out.error, std::abs(grad));
            if (std::abs(grad) < ops.tolerance) continue;

            // Hessian in this DOF
            T const h = la::dot(AV2.col(i), x); // O(m)
            // Newton step in this DOF, assuming the Hessian is non-singular
            T const scale = -grad / (h > 0 && std::isfinite(grad / h) ? h : 1);
            // Optimize the scale via bisection
            T const optimized_scale = optimize_scale<T>(grad, scale, ops.tolerance, [&](T scale) {
                logx1 = logx + scale * AV.col(i); // O(m)
                x1 = la::exp(logx1); // O(m)
                c = A.t() * x1; // O(m n)
                if (c.is_finite() && c.min() > 0) return la::dot(AV.col(i), x1) - V1(i); // O(m)
                else return std::numeric_limits<T>::infinity();
            });
            y1 = y + optimized_scale * V.col(i);
            x = la::exp(logx1 = A * y1 + logq); // O(m n); Ideally you could do this in O(m) but precision is an issue.
            c = A.t() * x;
            if (c.min() > 0 && c.is_finite()) {
                logx = logx1;
                y = y1;
            } else { // Occasionally the optimized bisection objective is finite and the unoptimized is non-finite
               T const optimized_scale = optimize_scale<T>(grad, scale, ops.tolerance, [&](T scale) {
                    logx1 = A * (y + scale * V.col(i)) + logq;
                    x1 = la::exp(logx1);
                    c = A.t() * x1;
                    if (c.min() > 0 && c.is_finite()) return std::numeric_limits<T>::infinity();
                    return la::dot(AV.col(i), x1) - V1(i); // O(m)
                });
                y += optimized_scale * V.col(i);
                x = la::exp(logx = A * y + logq);
                if (!x.is_finite()) throw std::runtime_error("non-finite values in equilibrium concentration solver");
            }
        }
        if (out.error < ops.tolerance) {out.converged = true; break;}
        // print(out.error, (A.t() * x).t());
    }
...
/**
 * @brief Solve equilibrium concentrations with the dogleg method used in NUPACK 3
 * The objective is Objective(y) = 1^T exp(A y + q) - y^T s0
 A^T exp(A y + q) - s0
...
template <class T>
Output<T> equilibrate_finite(la::Mat<T> A, la::Col<T> const &logb, la::Col<T> const &logq, Options ops) {
    ...
    // Normalize the strand concentrations so relative error is solved for
    la::Col<T> b = la::exp(logb);
    A.each_row() /= b.t();
    b.ones();

    if (ops.initial_dual_guess.empty())
        ops.initial_dual_guess = initial_dual_guess(A, logq, la::Col<T>(A * b));
    ...
    auto out = [&]{switch (ops.method) {
        case Method::cd: return equilibrate_cd<T>(A, logq, ops);
        case Method::fit: return equilibrate_lse<T>(A, logb, logq, ops); // doesn't work that well
        case Method::dogleg: return equilibrate_gradient(A, b, logq, ops); // NUPACK 3 version
    }}();
    out.dual_solution %= la::exp(-logb);
    return out;
}
...
la::Col<T> initial_guess(la::Mat<T> const &A, la::Col<T> const &logq, la::Col<T> const &x0) {
    ...
    while (true) {
        ...
        if (!la::solve(y, AwA, A.t() * (weight % (la::log(x0) - logq)), la::solve_opts::likely_sympd + la::solve_opts::refine)) {
            std::stringstream ss;
        ...
        for (la::uword i = 0; i != A.n_cols; ++i) {
            ...
            if (logc(j) < -300 || logc(j) > 300) {bad = true; weight(j) *= 2;}
        }
        if (!bad) return y;
    }
}
