...
template <class T>
constexpr T const LogOf2 = 0.6931471805599453;

template <class T>
struct SingleDispatch<Big<T>> {
    static Big<T> exp(Big<T> const &t) noexcept {return {std::exp(T(t)), 0};}
    static T log(Big<T> const &t) noexcept {return std::log(t.mantissa) + t.exponent * LogOf2<T>;}
...
struct DoubleDispatch<U, Big<T>> {
    ...
    static void set_logarithm(U &u, Big<T> const &t) noexcept {
        u = std::log(static_cast<std::common_type_t<T, U>>(t.mantissa)) + t.exponent * LogOf2<std::common_type_t<T, U>>; // use higher precision type
    }
};
...
void map(O &&out, int i, int stop, F &&f) noexcept {
    if constexpr(!has_exponent<no_qual<O>>) {
