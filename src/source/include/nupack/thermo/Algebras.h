...
struct BigAlgebra : AlgebraBase<Rig> {
    ...
    static auto evaluate_mantissa(T const &t, E const &e) noexcept {return Rig::ldexp()(t, e);}

    template <class T, class E, NUPACK_IF(std::is_invocable_v<T const &, E const &>)>
    static auto evaluate_mantissa(T const &t, E const &e) noexcept {return t(e);}

    template <class T, class E>
    static auto evaluate_mantissa(Big<T> const &t, E const &e) noexcept {return std::ldexp(t.mantissa, t.exponent + e);}////Rig::ldexp()(t, e);

    template <class T, class E>
    static auto evaluate_mantissa(std::optional<T> const &t, E const &e) noexcept {return t ? evaluate_mantissa(*t, e) : Rig::zero();}////Rig::ldexp()(t, e);
...
struct BackwardAlgebra : AlgebraBase<Rig> {
    ...
    static bool recurse(Address<R> const &t, type s, Q const &q, Ts const &...ts) noexcept {return q(s, t, ts...);}

    template <class F, class Q, class T, class ...Ts, NUPACK_IF(std::is_base_of_v<Addressed, F>)>
    static bool recurse(F const &fun, type s, Q const &q, T const &t, Ts const &...ts) noexcept {
        return recurse(t, BackwardAlgebra().times(s, fun.value), q, ts..., fun.address());
    }

    template <class F, class Q, NUPACK_IF(std::is_base_of_v<Addressed, F>)>
    static bool recurse(F const &fun, type s, Q const &q) noexcept {
        return q(BackwardAlgebra().times(fun.value, s), fun.address());
    }

    template <class F, class Q, class ...Ts, NUPACK_IF(std::is_invocable_v<F, type, Q, Ts...>)>
    static bool recurse(F const &fun, type s, Q const &q, Ts const &...ts) noexcept {return fun(s, q, ts...);}

    template <class F, class Q, class ...Ts>
    static bool recurse(std::optional<F> const &fun, type s, Q const &q, Ts const &...ts) noexcept {
        return fun ? recurse(*fun, s, q, ts...) : False();
    }

    template <class F, class Q, NUPACK_IF(is_numeric<F>::value)>
    static bool recurse(F const &f, type s, Q const &q) noexcept {return q(BackwardAlgebra().times(s, f));}

    template <class F, class Q, class T, class ...Ts, NUPACK_IF(is_numeric<F>::value)>
    static bool recurse(F const &f, type s, Q const &q, T const &t, Ts const &...ts) noexcept {
        return recurse(t, BackwardAlgebra().times(s, f), q, ts...);
    }
    ...
    auto total(T const &t, F &&map) const noexcept {
        return [=](type s, auto const &q, auto const &...factors) noexcept -> bool {
            for (auto const &i : t) if (recurse(map(i), s, q, factors...)) return true;
            return false;
        };
    }
    ...
    auto dot(T const &t, Ts const &...ts) const noexcept {
        return [=](type s, auto const &q, auto const &...factors) noexcept -> bool {
            for (auto const i : indices(t))
                if (BackwardAlgebra().product(t(i), ts(i)...)(s, q, factors...)) return true;
            return false;
        };
    }
