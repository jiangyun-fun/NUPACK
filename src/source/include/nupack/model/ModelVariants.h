...
struct MinDangles {
    ...
    static auto has_backtrack() {return True();}
    static auto has_pair_probability() {return True();}
    template <class T>
...
// Mixed Materials
struct MixedNoStacking {
    static auto has_backtrack() {return True();}
    static auto has_pair_probability() {return True();}
};
struct MixedStacking {
    static auto has_backtrack() {return True();}
    static auto has_pair_probability() {return True();}
};
...
struct DangleFunction<NoStacking, P> : NoStacking {
    unsigned n;
    DangleFunction(P const &pset) : n(pset.alphabet.length()) {}
    unsigned alphabet_length() const {return n;}
...
value_type_of<M> stacking_energy(Stacking, M const &model, V const &v, int const nick) {
    // Use at least double precision for stacking exponentiation
    return inverse_boltzmann(model.beta, stacking_sum<std::common_type_t<real, value_type_of<M>>>(v, nick, model));
}
