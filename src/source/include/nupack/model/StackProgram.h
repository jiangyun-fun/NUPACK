...
template <class M, class T, bool PartitionFunction>
struct StackModel {
    M const &model;
    ...
    constexpr type boltz(type e) const {return PartitionFunction ? model.boltz(e) : e;}
    ...
    type one() const {return PartitionFunction ? 1 : 0;}
    type zero() const {return PartitionFunction ? 0 : inf<type>();}
};

// Returns a StackModel for a given sequence  v[s0]
template <class T, bool B, class V, class M>
StackModel<M, T, B> stack_model(V const &v, std::size_t s0, M const &model) {
...
T stacking_sum(V const &v, int const nick, M const &model) {
    ...
    A q = factor<A>(len(back(v)), len(front(v)), nick == 0, stack_model<T, true>(v, 0, model));
    for (auto s : range(1, len(v)))
        q = q * factor<A>(len(v[s-1]), len(v[s]), nick == s, stack_model<T, true>(v, s, model));
    return q.trace();
}
...
T minimum_stacking_energy(V const &v, int const nick, M const &model) {
    ...
    A q = factor<A>(len(back(v)), len(front(v)), nick == 0, stack_model<T, false>(v, 0, model));
    for (auto s : range(1, len(v)))
        q = q * factor<A>(len(v[s-1]), len(v[s]), nick == s, stack_model<T, false>(v, s, model));
    return q.trace();
}
