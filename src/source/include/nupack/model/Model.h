...
struct ModelConditions {
    ...
    NUPACK_REFLECT(ModelConditions, temperature, na_molarity, mg_molarity, immutable);
...
struct Model : MemberOrdered {
    ...
    NUPACK_REFLECT(Model, parameters, beta, has_terminal_penalty, ensemble);
    ...
    template <class U>
    explicit Model(Model<U> const &o) : parameters(o.parameters), beta(o.beta),
        ensemble(o.ensemble), has_terminal_penalty(o.has_terminal_penalty) {
    ...
    T temperature() const {return 1 / beta / Kb;}
    ...
    auto closing_pairs(Base i) const {return pairing().closing_pairs(i);}
...
Model<T>::Model(Ensemble e, ParameterSet<T> p, ModelConditions const &cs, Optional<BasePairing> pairs)
    : parameters(std::move(p)), beta(1.0 / (Kb * cs.temperature)), ensemble(e) {
...
Model<T>::Model(Ensemble e, ParameterFile const &p, ModelConditions const &cs, Optional<BasePairing> pairs)
    : Model(e, ParameterSet<T>(load_parameter_set(ParameterInfo{.file=p, .kind="dG", .na_molarity=cs.na_molarity, .mg_molarity=cs.mg_molarity, .temperature=cs.temperature, .immutable=cs.immutable})), cs, std::move(pairs)) {}
...
T Model<T>::interior_energy(S1 const &a, S2 const &b) const {
    ...
    if (n1 == 0 && n2 == 0) { // Stack loop
        ...
        if (sz == 1) {
            ...
            return en + dG(parameters.stack(), +front(a), +back(a), +front(b), +back(b)) - dG(parameters.loop_bias());
        } else {
    ...
    if ((n1 == 1 && ((parameters.is_condensed && n2 > 1) || (!parameters.is_condensed && n2 > 2)))
     || (n2 == 1 && ((parameters.is_condensed && n1 > 1) || (!parameters.is_condensed && n1 > 2)))) {
