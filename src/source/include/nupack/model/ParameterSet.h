...
#include "Constants.h" // for temperature
...
struct ParameterInfo {
    ...
    ParameterFile file;
    string kind = "dG";
    real na_molarity = 1, mg_molarity = 0;
    real temperature = DefaultTemperature;
    bool immutable;

    NUPACK_REFLECT(ParameterInfo, temperature, na_molarity, mg_molarity, kind, file, immutable);
};
...
struct ParameterIndex {
    ...
    auto loop_bias()         const {return ParameterDescriptor<5>();}
    auto ninio()             const {return ParameterDescriptor<6, 5>();}
    auto interior_size()     const {return ParameterDescriptor<11, 30>();}
    auto bulge_size()        const {return ParameterDescriptor<41, 30>();}
    auto hairpin_size()      const {return ParameterDescriptor<71, 30>();}

    constexpr auto variable_start() const {return hairpin_size().stop();}

    std::uint32_t alphabet_length;
    bool is_condensed;

    // Variable size arrays
    ParameterGrid<4> stack; // up to 357
    ParameterGrid<3> dangle5; // 421
    ParameterGrid<3> dangle3; // 485
    ParameterGrid<2> terminal_penalty; // 501
    ParameterGrid<4> coaxial_stack; // 757
    ParameterGrid<4> interior_mismatch; // 1013
    ParameterGrid<2> interior_mismatch_1; // 1029
    ParameterGrid<4> terminal_mismatch; // 1285
    ParameterGrid<4> hairpin_mismatch; // 1541
    ParameterGrid<6> hairpin_tetra; // 5637
    ParameterGrid<5> hairpin_tri; // 6661
    // These are optional!
    ParameterGrid<6> interior_1_1; // 10757
    ParameterGrid<7> interior_1_2; // 27141
    ParameterGrid<8> interior_2_2; // 92677
...
/*
Earlier there was some caching implementation for copying parameter sets between different data types
But this was removed. Copying Model<float> from Model<real> takes only about 1e-5 seconds so not that significant
Caching could be used to avoid loading the same parameter file over and over but this is not implemented right now
*/
...
struct ParameterSet : ParameterBase {
    ...
    explicit ParameterSet(ParameterSet<U> o) : base_type(std::move(static_cast<base_type &>(o))),
        data(std::move(o.data)) {static_assert(!is_same<T, U>, "Should use normal copy constructor");}
    ...
    NUPACK_EXTEND_REFLECT(ParameterSet, ParameterBase, data);
