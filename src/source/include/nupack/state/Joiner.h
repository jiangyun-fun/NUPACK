...
inline std::size_t stacking_propensity_vector_size(std::size_t n_bases, std::size_t n_basepairs) {
    std::size_t const n_dangles = n_bases + 1;
    return n_bases * n_dangles * n_dangles + 2 * n_bases + 4 * n_bases * n_dangles + 2 * n_basepairs;
}
...
struct ProductJoiner : TotallyOrdered {
    ...
    std::size_t propensity_vector_size(M const &model, RF const &rate_function) const {
        if (!has_subensemble(model.ensemble)) return len(base_pairs) * sq(rate_function.number_of_bimolecular_half_contexts());
        else return stacking_propensity_vector_size(model.alphabet().length(), len(base_pairs)) * sq(rate_function.number_of_bimolecular_half_contexts());
    }
};
...
void for_join_sites_in_complex(State &&w, X const &x, uint p, uint q, Base b, Base c, F &&f) {
    ...
    fork(w.rate_function, [&](auto const &rf) {
        for_exterior_loops_in_complex(w, x, [&](auto &&o) {
            ...
            for_join_locs_in_loop(o.seqs, p, q, b, c, w.model, rf, [&](auto &&m) {
...
void ProductJoiner::initialize_joiner(State const &w) {
    ...
    std::size_t const n = propensity_vector_size(w.model, w.rate_function);
