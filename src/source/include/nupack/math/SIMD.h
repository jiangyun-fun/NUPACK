...
// #define NUPACK_SIMD_EVAL(x) ([](auto y){NUPACK_ASSERT(is_finite(y), y); return y;}((x).eval()))
...
// Optimal number of elements in a SIMD pack, 0 if SIMD is not enabled for this type
template <class T>
struct OptimalSize {static constexpr std::size_t value = 0;};

template <class T>
static constexpr auto optimal_size = OptimalSize<T>::value;

template <class T>
static constexpr std::uintptr_t alignment = optimal_size<T> ? sizeof(T) * optimal_size<T> : alignof(T);

// Use SIMD aligned allocator if SIMD is to be used
template <class T>
using allocator = if_t<optimal_size<T> != 0, boost::alignment::aligned_allocator<T, alignment<T>>, std::allocator<T>>;
...
NUPACK_DETECT(is_simd, decltype(simdpp::reduce_add(std::declval<T>().eval())));
    ...
    static O impl(Chunk<N> i, T *t, std::index_sequence<Is...>) noexcept {
        try {
            if constexpr(std::is_integral_v<T> && std::is_signed_v<T>) {
                return NUPACK_SIMD_EVAL(O(simdpp::make_int(t[i.value - int(Is)]...)));
            } else if constexpr(std::is_integral_v<T> && !std::is_signed_v<T>) {
                return NUPACK_SIMD_EVAL(O(simdpp::make_uint(t[i.value - int(Is)]...)));
            } else if constexpr(std::is_floating_point_v<T>) {
                return NUPACK_SIMD_EVAL(O(simdpp::make_float(t[i.value - int(Is)]...)));
            } else {
                static_assert(T::no_load_possible_for_this_type);
            }
        } catch (...) {
            print("values", i, t[i.value - int(Is)]...);
            throw;
        }
    }
...
struct DoubleDispatch<X, T *, std::enable_if_t<traits::is_simd<X>>> {
    static void store(X const &x, T *t) noexcept {simdpp::store(t, x);}
};
...
constexpr auto max_of(T const &t, U const &u, Ts const &...ts) noexcept {
    if constexpr(sizeof...(Ts) == 0) return max(t, u);
    else return max_of(max(t, u), ts...);
}
...
template <class T>
struct is_finite_functor_t<T, void_if<simd::traits::is_simd<T>>> {
    bool operator()(T const &t) const {return all_of(math::unpack(t), is_finite);}
};

template <class T>
struct is_nonnegative_functor_t<T, void_if<simd::traits::is_simd<T>>> {
    bool operator()(T const &t) const {return all_of(math::unpack(t), is_nonnegative);}
};


namespace io {

NUPACK_PRINT_IF(simd::traits::is_simd<T>) {
    void operator()(std::ostream &os, T const &t, Indent id) const {Printer<decltype(math::unpack(t))>()(os, math::unpack(t), id);}
};
