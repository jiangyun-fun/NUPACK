...
struct BasePairing {
    ...
    small_vec<small_vec<Base, 4>, 4> possible_pairs, possible_closing_pairs;

    std::uint32_t length;
    BasePairing() = default;
    BasePairing(std::uint32_t n) : pairing(n * n), closing(n * n), 
        possible_pairs(n), possible_closing_pairs(n), length(n) {}

    NUPACK_REFLECT(BasePairing, pairing, closing, possible_pairs, possible_closing_pairs);
    ...
    auto closing_pairs(Base b) const noexcept {return view(possible_closing_pairs[+b]);}
