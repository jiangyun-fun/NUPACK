...
struct NamedSequence : Sequence, CompareByKey<NamedSequence> {
    using base_type = Sequence;
    using is_member_ordered = False;
...
struct NamedDomain : Domain, CompareByKey<NamedDomain> {
...
struct TargetStrand {
    ...
    NUPACK_REFLECT(TargetStrand, name, domains);
...
struct TargetComplex {
    ...
    NUPACK_REFLECT(TargetComplex, name, strands, structure, bonus);
...
struct TargetTube {
    ...
    NUPACK_REFLECT(TargetTube, name, complexes, concentrations, n_on_targets);
};
