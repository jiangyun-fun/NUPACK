...
// Upper triangle type that may include the diagonal
...
struct Triangle {
    ...
    template <class F>
    Triangle(uint n, F &&f) : Triangle(n) {
        for (auto j : range(length())) for (auto i : range(j + Inclusive))
            (*this)(i, j) = f(i, j);
    }
...
struct Square {
    ...
    T & operator()(uint i, uint j) {
        ...
        return values[i + j * m_length];
    }

    T const & operator()(uint i, uint j) const {return const_cast<Square &>(*this)(i, j);}
};
