...
//     vec<int> positions; // ending position of each sequence (0 is not included)
...
//     auto strands() const {auto v = views(); return SequenceList(v.begin(), v.end());}
//     bool multi() const {return n_strands() > 1;}

//     auto slice(iseq b, iseq e) const {
//         vec<int> positions2{view(positions, b, e)};
...
using Complex = SequenceList;
using ComplexView = View<Sequence const *>;

using Nicks = vec<iseq, 4>;

template <class V>
Nicks complex_nicks(V const &v, bool include_zero=false) {return prefixes<Nicks>(include_zero, indirect_view(v, len));}

// class Complex : public ComplexView<false> {
//     Complex(Strand &&s, std::size_t n) : base_type{std::move(s), vec<int>(1u, n)} {}
// public:
//     using base_type = typename Complex::ComplexView;

//     Complex() = default;

//     template <class V=SequenceList, NUPACK_IF(!std::is_base_of_v<Complex, V> && can_construct<Strand, value_type_of<V>>)>
//     explicit Complex(V const &v, bool b=true) : base_type{join<Strand>(v), prefixes<vec<int>>(false, indirect_view(v, len))} {}

//     Complex(SequenceList const &v) : Complex(v, true) {}
...
namespace std {
    template <> struct hash<nupack::Complex> {
        size_t operator()(nupack::Complex const &m) const noexcept;
    };
}
