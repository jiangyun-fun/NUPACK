...
using BasePairList = small_vec<same_pair<Base>, 8>;
...
auto stack_left(V const &v, int const nick, int pair, M const &model) {
    ...
    if (pair < nick) {
        for (auto s : range(nick + 1, len(v)))
            x = x * factor<A>(len(v[s-1]), len(v[s]), false, stack_model<T, true>(v, s, model));
...
auto stack_right(V const &v, int const pair, int nick, M const &model) {
    ...
    if (nick < pair) {
        for (auto s : ~range(nick))
            x = factor<A>(len(v[s-1]), len(v[s]), false, stack_model<T, true>(v, s, model)) * x;
...
template <class M>
struct EndStackModel : StackModel<M, real, true> {
    EndStackModel(StackModel<M, real, true> const &o) : StackModel<M, real, true>(o) {}
...
struct FullStackEnvironment {
        ...
        if (seq != nick) {
            if (nick < seq) {
                ...
                for (auto s : range(seq))
                    x = x * factor<A>(len(v[s ? s-1 : len(v)-1]), len(v[s]), false, stack_model<T, true>(v, s, model));
            }
            x = x * factor<A>(len(v[seq ? seq-1 : len(v)-1]), base+1, false, stack_model<T, true>(v, seq, model));
        }
        return x;
    }
    ...
    auto end_stack(V const &v, int nick, int seq, int base, M const &model) const {
        ...
        if (seq1 != nick) {
            if (1 < (len(v) + nick - seq1) % len(v)) {
                ...
                if (seq2 < nick) {
                    for (auto s : ~range(seq2, nick))
                        x = factor<A>(len(v[s ? s-1 : len(v)-1]), len(v[s]), false, stack_model<T, true>(v, s, model)) * x;
                } else {
                    for (auto s : ~range(nick))
                        x = factor<A>(len(v[s ? s-1 : len(v)-1]), len(v[s]), false, stack_model<T, true>(v, s, model)) * x;
            ...
            x = factor<A>(len(v[seq])-base, len(v[seq1]), false, EndStackModel<M>(stack_model<T, true>(v, seq1, model))) * x;
        }
...
struct StackEnvironment {
    ...
    template <class V, class M>
    StackEnvironment(V const &v, int const nick, M const &model) {
        ...
        // every pair after the nick but before seq
        for (auto s : range(nick+1, len(v)))
            bcache.emplace_back(bcache.back() * factor<A>(len(v[s-1]), len(v[s]), false, stack_model<T, true>(v, s, model)));
        for (auto s : range(nick))
            bcache.emplace_back(bcache.back() * factor<A>(len(v[s ? s-1 : len(v)-1]), len(v[s]), false, stack_model<T, true>(v, s, model)));

        for (auto s : ~range(nick)) // progress backwards from the nick
            ecache.emplace_back(factor<A>(len(v[s ? s-1 : len(v)-1]), len(v[s]), false, stack_model<T, true>(v, s, model)) * ecache.back());
    ...
    B begin_stack(V const &v, int const nick, int seq, int base, M const &model) const {
        ...
        return prefix * factor<A>(len(v[seq ? seq-1 : len(v)-1]), base+1, false, stack_model<T, true>(v, seq, model));
    }
    ...
    B end_stack(V const &v, int nick, int seq, int base, M const &model) const {
        ...
        return factor<A>(len(v[seq])-base, len(v[seq1]), false, EndStackModel<M>(stack_model<T, true>(v, seq1, model))) * suffix;
    }
};
...
// Following would have been easier if using absolute indexing rather than iterating through o ...

// Left column for base i of sequence s to bind from exterior loop v
template <class O, class A, class V, class M>
void accumulate_left_stacks(O const &o, A const &env, M const &model, BasePairList const &pairs, V const &v, uint nick, uint s, uint i) {
    ...
    auto const x = env.begin_stack(v, nick, s, i, model);
    auto const y = env.end_stack(v, nick, s, i, model);
    ...
    if (len(v[s]) == 3 && !front(v[s]).is_null() && !back(v[s]).is_null()) {
        for (auto j : bases) {
            if (model.pairing().can_close(bi, j)) // BP must stack left, BP after must stack left
                o.coaxial_both_left(j, x(1) * y(1) * coaxial_factor(model, true, back(v[s ? s-1 : len(v)-1]), front(v[s]), bi, j) * coaxial_factor(model, false, j, bi, back(v[s]), front(v[(s+1) % len(v)]))); 
            else o.skip();
        }
    } else o.skip(len(bases));
    ...
    for (auto bi_ : bases) {
        if (bi_ == bi) {
            ...
            o.coaxial_both_right(bi_, lo * hi);
        } else o.skip();
    }
    ...
    for (auto j : bases) {
        if (model.pairing().can_close(bi, j) && bi1.is_null() && !v[s][i+1].is_null()) {
            for_dangles(bases, [&](Base j1) {
                o.coaxial_top_left(j, j1, (x(0) * dangle(Base::null(), bi, j, j1) + x(1) * (bi0.is_null() ? 0 : dangle(bi0, bi, j, j1)))
                     * y(1) * coaxial_factor(model, false, j, bi, back(v[s]), front(v[(s+1) % len(v)]))); // BP must not stack left, BP after must stack left
            });
        } else o.skip(len(bases) + 1);
    }
    ...
    for (auto j : bases) {
        if (model.pairing().can_close(bi, j) && bi0.is_null() && !v[s][i-1].is_null()) {
            for_dangles(bases, [&](Base j0) {
                o.coaxial_bottom_left(j, j0, x(1) * coaxial_factor(model, true, back(v[s ? s-1 : len(v)-1]), front(v[s]), bi, j)
    ...
    for (auto bi_ : bases) {
        if (bi != bi_) o.skip(len(bases) + 1);
        else for_dangles(bases, [&](Base bi0_) {
            if (dangle_match(bi0_, bi0))
                o.coaxial_top_right(bi_, bi0_, x(!bi0_.is_null()) * (y(0) + (bi1.is_null() ? 0 : y(1)))); // BP must not stack left, BP after may only stack left if a dangle exists
            else o.skip();
        });
    }
    ...
    for (auto bi_ : bases) {
        if (bi != bi_) o.skip(len(bases) + 1);
            ...
            if (dangle_match(bi1_, bi1))
                o.coaxial_bottom_right(bi_, bi1_, x(0) * (y(0) + y(1) * (!bi1.is_null() && (bi1_.is_null() || i+3 != len(v[s])) ? 1 : 0))); // BP must not stack left, BP after may or may not stack left
            else o.skip();
        });
    }
    // Coaxial stack top left and bottom right
    if (bi1.is_null() && !v[s][i+1].is_null()) {
        izip(key_view(pairs), item_view(pairs), [&](auto index, auto bi_, auto bj) {
            if (bi == bi_) 
                o.coaxial_top_left_bottom_right(index, x(0) * y(1) * coaxial_factor(model, false, bj, bi, back(v[s]), front(v[(s+1) % len(v)]))); // BP must not stack left. BP after must stack left
            else o.skip();
        });
    } else o.skip(len(pairs));

    // Coaxial stack bottom left and top right
    if (bi0.is_null() && !v[s][i-1].is_null()) {
        izip(key_view(pairs), item_view(pairs), [&](auto index, auto bi_, auto bj) {
            if (bi == bi_)
                o.coaxial_bottom_left_top_right(index, x(1) * (y(0) + (bi1.is_null() ? 0 : y(1))) * coaxial_factor(model, true, back(v[s ? s-1 : len(v)-1]), front(v[s]), bi, bj)); // BP must stack. BP after may or may not stack left
            else o.skip();
        });
    } else o.skip(len(pairs));
}
...
template <class O, class A, class V, class M>
void accumulate_right_stacks(O const &o, A const &env, M const &model, BasePairList const &pairs, V const &v, uint nick, uint s, uint j) {
    auto const bases = model.alphabet().all();
    Base const bj = v[s][j];
    Base const bj1 = j+2 < len(v[s]) ? v[s][j+1] : Base::null();
    Base const bj0 = 1 < j ? v[s][j-1] : Base::null();

    auto x = env.begin_stack(v, nick, s, j, model);
    // This gives the pfunc of every BP up to the new one, depending if the one after the new one stacks or not to the left
    auto y = env.end_stack(v, nick, s, j, model);

    // No coaxial stacks
    for (auto bj_ : bases) {
        if (bj != bj_) o.skip(sq(len(bases) + 1));
        else for_dangles(bases, [&](Base bj1_) {
            if (!dangle_match(bj1_, bj1)) o.skip(len(bases) + 1);
                ...
                if (dangle_match(bj0_, bj0)) {
                    ...
                    o.both_dangle(bj_, bj1_, bj0_, lo * hi);
                } else o.skip();
            });
        });
    }
    ...
    // Coaxial stacks on left, top and bottom
    for (auto bj_ : bases) {
        if (bj_ == bj) o.coaxial_both_left(bj_, x(0) * (y(0) + (bj1.is_null() ? 0 : y(1)))); // BP cannot stack left. BP after can stack left only if dangle exists.
        else o.skip();
    }
    ...
    for (auto bi : bases) {
        if (model.pairing().can_close(bi, bj) && len(v[s]) == 3 && !front(v[s]).is_null() && !back(v[s]).is_null()) {
            o.coaxial_both_right(bi, x(1) * y(1) * coaxial_factor(model, true, back(v[s ? s-1 : len(v)-1]), front(v[s]), bj, bi)
    ...
    // Coaxial stack on top left only
    for (auto bj_ : bases) {
        if (bj != bj_) o.skip(len(bases) + 1);
        else for_dangles(bases, [&](Base j1) {
            if (dangle_match(j1, bj1)) {
                o.coaxial_top_left(bj_, j1, x(0) * (y(0) + y(1) * (bj1.is_null() || (j+3 == len(v[s]) && !j1.is_null()) ? 0 : 1)));
            } else o.skip();
        });
    }
    ...
    for (auto bj_ : bases) {
        if (bj != bj_) o.skip(len(bases) + 1);
            ...
            if (dangle_match(j0, bj0)) {
                o.coaxial_bottom_left(bj_, j0, x(j0.is_null() ? 0 : 1) * (y(0) + (bj1.is_null() ? 0 : y(1))));
            } else o.skip();
        });
    }
        ...
        for (auto bi : bases)
            if (model.pairing().can_close(bj, bi)) {
                for_dangles(bases, [&](Base bi0_) {
                    o.coaxial_top_right(bi, bi0_, x(1) * coaxial_factor(model, true, back(v[s ? s-1 : len(v)-1]), front(v[s]), bj, bi)
    ...
    } else o.skip(len(bases) * (len(bases) + 1));
        ...
        for (auto bi : bases)
            if (model.pairing().can_close(bj, bi)) {
                for_dangles(bases, [&](Base bi1_) {
                    o.coaxial_bottom_right(bi, bi1_, (x(0) * dangle(Base::null(), bj, bi, bi1_) + (bj0.is_null() ? 0 : x(1) * dangle(bj0, bj, bi, bi1_)))
    ...
    // Coaxial stack top left and bottom right
    izip(key_view(pairs), item_view(pairs), [&](auto index, auto bi, auto bj_) {
        if (bj == bj_ && bj1.is_null() && !back(v[s]).is_null()) {
            o.coaxial_top_left_bottom_right(index, x(0) * y(1) * coaxial_factor(model, false, bi, bj, back(v[s]), front(v[(s+1) % len(v)])));
        } else o.skip();
    });
    ...
    izip(key_view(pairs), item_view(pairs), [&](auto index, auto bi, auto bj_) {
        if (bj == bj_ && j == 1 && !front(v[s]).is_null()) {
            o.coaxial_bottom_left_top_right(index, x(1) * coaxial_factor(model, true, back(v[s ? s-1 : len(v)-1]), front(v[s]), bj, bi)
